# FORM SS-4
# Application for Employer Identification Number (EIN)
# (For use by employers, corporations, partnerships, trusts, estates, churches, government agencies, Indian tribal entities, certain individuals, and others.)

## SECTION 1: ENTITY INFORMATION

**1. Legal name of entity (or individual) for whom the EIN is being requested**
OCOOL INC.

**2. Trade name of business (if different from name on line 1)**
OCOOL

**3. Executor, administrator, trustee, "care of" name**
[Not applicable]

**4a. Mailing address (room, apt., suite no. and street, or P.O. box)**
1013 Centre Road

**4b. City, state, and ZIP code (if foreign, see instructions)**
Wilmington, DE 19805

**5a. Street address (if different) (Do not enter a P.O. box.)**
[Same as mailing address]

**5b. City, state, and ZIP code (if foreign, see instructions)**
Wilmington, DE 19805

**6. County and state where principal business is located**
New Castle County, Delaware

**7a. Name of responsible party**
[Responsible Party Name]

**7b. SSN, ITIN, or EIN**
[Responsible Party SSN/EIN]

**8a. Is this application for a limited liability company (LLC)?**
☐ Yes  ☑ No

**8b. If "Yes," enter the number of LLC members**
N/A

**8c. If "Yes," was the LLC organized in the United States?**
☐ Yes  ☐ No

**9a. Type of entity (check only one box)**
☑ Corporation (enter form number to be filed) ► 1120

**9b. If a corporation, name the state or foreign country (if applicable) where incorporated**
Delaware

**10. Reason for applying (check only one box)**
☑ Started new business (specify type) ► Technology/Software Services
☐ Hired employees (Check the box and see line 13.)
☐ Compliance with IRS withholding regulations
☐ Changed type of organization (specify new type) ►
☐ Purchased going business
☐ Created a trust (specify type) ►
☐ Created a pension plan (specify type) ►
☐ Other (specify) ►

**11. Date business started or acquired (month, day, year)**
[Date]

**12. Closing month of accounting year**
December

**13. Highest number of employees expected in the next 12 months**
Number of household employees: 0
Number of agricultural employees: 0
Number of other employees: [Number]

**14. Check one box that best describes the principal activity of your business**
☑ Other (specify) ► Software Development and Technology Services

**15. Indicate principal line of merchandise sold, specific construction work done, products produced, or services provided**
Software development, technology consulting, and related services

**16. Has the applicant entity shown on line 1 ever applied for and received an EIN?**
☐ Yes  ☑ No

**If "Yes," write previous EIN here:** ►

---

## THIRD PARTY DESIGNEE
Complete this section only if you want to authorize the named individual to receive the entity's EIN and answer questions about the completion of this form.

**Designee's name:** [Name]
**Designee's telephone number (include area code):** ( ) -
**Designee's fax number (include area code):** ( ) -
**Address and ZIP code:** [Address]

---

## APPLICANT'S SIGNATURE

Under penalties of perjury, I declare that I have examined this application, and to the best of my knowledge and belief, it is true, correct, and complete.

**Name and title (type or print clearly):** ________________________

**Signature:** ________________________  **Date:** __________

**Applicant's telephone number (include area code):** ( ) -
**Applicant's fax number (include area code):** ( ) -

---

**For Privacy Act and Paperwork Reduction Act Notice, see separate instructions.**
**Form SS-4 (Rev. December 2017)**
**Department of the Treasury - Internal Revenue Service**

---

## NOTES:
- This form should be submitted to the IRS via fax, mail, or online at IRS.gov
- Processing time: Immediate (online), 1-2 weeks (fax), 4-5 weeks (mail)
- Keep a copy for your records
